    ...
    internal static class IconTests
    {
        ...
        private static int Run(string[] args)
        {
            ...
            Check(BitConverter.ToUInt16(exe, pe + 4) == 0x8664 && BitConverter.ToUInt16(exe, pe + 24 + 68) == 2,
                "PE x64 Windows GUI");
            Check(AssemblyName.GetAssemblyName(args[1]).Version.ToString() == "2.13.0.0", "assembly version 2.13.0.0");
            var version = FileVersionInfo.GetVersionInfo(args[1]);
            Check(version.FileVersion == "2.13.0.0" && version.ProductName == "曜核", "EXE version and product display name");
